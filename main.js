    ...
    andy.ev.on("group-participants.update", async anu => {
        ...
        try {
            ...
            for (let num of participants) {
                ...
                if (anu.action == "add") {
                    andy.sendMessage(anu.id, { text: check ? `@${anu.author.split("@")[0]} menambahkan @${num.split("@")[0]}` : `Welcome @${num.split("@")[0]}`, contextInfo: { mentionedJid: [...tag], externalAdReply: { thumbnailUrl: ppuser, title: "© dyvsss", body: "", renderLargerThumbnail: true, sourceUrl: linkgc, mediaType: 1 } } });
                }
                if (anu.action == "remove") {
                    andy.sendMessage(anu.id, { text: check ? `@${anu.author.split("@")[0]} mengeluarkan @${num.split("@")[0]}` : `@${num.split("@")[0]} telah keluar`, contextInfo: { mentionedJid: [...tag], externalAdReply: { thumbnailUrl: ppuser, title: "© dyvsss", body: "", renderLargerThumbnail: true, sourceUrl: linkgc, mediaType: 1 } } });
                }
                if (anu.action == "promote") {
                    andy.sendMessage(anu.id, { text: `@${anu.author.split("@")[0]} menjadikan @${num.split("@")[0]} sebagai admin`, contextInfo: { mentionedJid: [...tag], externalAdReply: { thumbnailUrl: ppuser, title: "© dyvsss", body: "", renderLargerThumbnail: true, sourceUrl: linkgc, mediaType: 1 } } });
                }
                if (anu.action == "demote") {
                    andy.sendMessage(anu.id, { text: `@${anu.author.split("@")[0]} memberhentikan @${num.split("@")[0]} sebagai admin`, contextInfo: { mentionedJid: [...tag], externalAdReply: { thumbnailUrl: ppuser, title: "© dyvsss", body: "", renderLargerThumbnail: true, sourceUrl: linkgc, mediaType: 1 } } });
                }
            }
        } catch (err) {
            console.log(err);
